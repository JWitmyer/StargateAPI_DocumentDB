...
{
    public class CreateAstronautCommand : IRequest
    {
        public Astronaut? astronaut { get; set; } 
    } 
    public class CreateAstronautCommandHandler : IRequestHandler<CreateAstronautCommand >
    {
        readonly IRepository<Astronaut> _repo;


        public CreateAstronautCommandHandler(IRepository<Astronaut> repo)
        {
            _repo = repo;

             
        }
        public async Task Handle(CreateAstronautCommand request, CancellationToken cancellationToken)
        {
            ...
            request.astronaut.PartitionKey = "f60710cc-084b-4fd0-9291-d8f37d682121"; //At this point could just be the word Astronauts

            await _repo.AddAsync(request.astronaut);
